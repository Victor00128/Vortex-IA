...
const ChatInput: React.FC<ChatInputProps> = ({ onSendMessage, isLoading, isImageModel }) => {
  ...
  return (
    <div className="bg-zinc-900/70 backdrop-blur-sm p-3 md:p-4 border-t border-zinc-700">
        ...
        {file && (
            <div className="mb-2 flex items-center justify-between bg-zinc-800 p-2 rounded-lg text-sm">
                ...
                <button 
                    ...
                    className="text-red-500 hover:text-red-400 font-bold ml-2 text-lg"
        ...
        <div className="flex items-center gap-2 md:gap-4">
            {!isImageModel && (
                <button
                    ...
                    disabled={isLoading}
                    className="p-2 md:p-3 text-gray-400 hover:text-white transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
            ...
            <input
                ...
                disabled={isLoading}
                className="flex-1 bg-zinc-800 border border-zinc-700 rounded-full py-2 md:py-3 px-4 md:px-5 text-sm md:text-base text-black placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500 transition duration-300 disabled:opacity-50"
                autoComplete="off"
            />
            <button
                type="submit"
                disabled={isLoading || (!input.trim() && !file)}
                className="bg-red-600 text-white rounded-full p-2 md:p-3 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:ring-offset-zinc-900 transition duration-300 disabled:bg-zinc-700 disabled:cursor-not-allowed flex-shrink-0"
